...
pic_output = 'C:\\Users\\Administrator\\Desktop\\type_'  # 聚类图文件名前缀
...
for i in range(k):  # 逐一作图，作出不同样式
    ...
    plt.xticks(range(1, 5), xlabels, rotation=10)  # 坐标标签
    plt.title('商圈类别%s' % (i + 1))  # 我们计数习惯从1开始
    plt.subplots_adjust(bottom=0.15)  # 调整底部
    plt.savefig('%s%s.png' % (pic_output, i + 1))  # 保存图片